...
\begin{table*}[!h]
\centering
\caption{Overview of the Datasets. We collect five datasets, in which, industry series sets were real-world flows, and the remaining of three were simulated data. The real-world traffic were labeled as benign because the security device never trigger any alert in an enterprise. These data are captured by tcpdump and then using the scapy module to extract the HTTP header information we need.}
\label{tbl:db_02}
...
For evaluate the robustness, we add the real-world traffic into each simulated data. We compare the effective performance between our system and DECANTeR \cite{bortolameotti2017decanter} against different datasets. The accuracy is calculated by equation (\ref{eq:accuracy}) and the detail result will show below.}
\label{tbl:db_03}
